        ...
        surfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                ...
                try {
                    ...
                    Log.e("DIM_V", intsCon(viewWidth,viewHeight));
                    Log.e("DIM_C", intsCon(camWidth,camHeight));
                    Log.e("DIM_N", intsCon(newWidth,newHeight));
...
//                    surfaceView.requestLayout();
                    getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Log.e("NOPE",intsCon(surfaceViewWidth,surfaceViewHeight));
                            Log.e("NOPE_C1", intsCon(cameraSource.getPreviewSize().getWidth(),cameraSource.getPreviewSize().getHeight()));
                            Log.e("NOPE_F1", intsCon(surfaceView.getWidth(),surfaceView.getHeight()));
                            Log.e("NOPE_MF1", intsCon(surfaceView.getMeasuredWidth(),surfaceView.getMeasuredHeight()));
                            surfaceView.getHolder().setFixedSize(surfaceViewWidth,surfaceViewHeight);
                            ViewGroup.LayoutParams params = surfaceView.getLayoutParams();
                            Log.e("NOPE_C", intsCon(cameraSource.getPreviewSize().getWidth(),cameraSource.getPreviewSize().getHeight()));
                            Log.e("NOPE_F", intsCon(surfaceView.getWidth(),surfaceView.getHeight()));
                            Log.e("NOPE_MF", intsCon(surfaceView.getMeasuredWidth(),surfaceView.getMeasuredHeight()));
                        }
                    });
                    Log.e("DIM_F", intsCon(surfaceView.getWidth(),surfaceView.getHeight()));
                    Log.e("DIM_MF", intsCon(surfaceView.getMeasuredWidth(),surfaceView.getMeasuredHeight()));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
...
//                            Paint boundRect = new Paint();
//                            boundRect.setColor(Color.GREEN);
//                            boundRect.setStrokeWidth(10);
//                            boundRect.setStyle(Paint.Style.STROKE);
//
//                            Canvas boundCanvas = new Canvas();
//                            boundCanvas.drawRect(bound,boundRect);
//                            getActivity().findViewById(R.id.cameraoverlay).draw(boundCanvas);
//                            cameraFrame.draw(boundCanvas);
                            break;
                        }
                    }
                    if (centerQR != null) {
